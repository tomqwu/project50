...
describe("POST /api/users/[id]/follow", () => {
  ...
  it("creates a follow edge and returns 201", async () => {
    ...
    expect(body.followerId).toBe(alice.id);
    expect(body.followeeId).toBe(bob.id);
  });

  it("returns 422 for self-follow", async () => {
    const alice = await createUser({ handle: "alice" });
    vi.mocked(requireUser).mockResolvedValue(alice.id);

    const res = await POST(new Request("http://localhost"), makeCtx(alice.id));
    expect(res.status).toBe(422);
    await expect(res.json()).resolves.toMatchObject({ error: "CANNOT_FOLLOW_SELF" });
  });
});
